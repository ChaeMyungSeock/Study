...
for i in range(700):
    ...
    model = Sequential()
    model.add(Conv1D(256,kernel_size=3, padding = 'same', activation = leaky, input_shape = (5,4))) 
    model.add(Flatten())
    model.add(Dense(300, activation = leaky))
    model.add(Dense(4, activation = leaky))

    earlystopping = EarlyStopping(monitor= 'loss', patience=5, mode = 'min')
    model.compile(optimizer='adam', loss = 'mse',metrics=['mse'])
    model.fit(x,y, epochs=50, batch_size=1, callbacks=[earlystopping],validation_split=0.1)
