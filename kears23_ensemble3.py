...
y2 = np.array([range(201,301), range(611,711), range(100)])

x3 = np.array([range(1,101), range(311, 411), range(411,511)])
x4 = np.array([range(711,811), range(711, 811), range(511,611)])

y3 = np.array([range(101,201), range(411,511), range(100)])
y4 = np.array([range(201,301), range(611,711), range(100)])
...
x1_train, x1_test, y1_train, y1_test,x2_train, x2_test, y2_train, y2_test, x3_train, x3_test, y3_train, y3_test,x4_train, x4_test, y4_train, y4_test   = train_test_split(x1,y1,x2,y2,x3,y3,x4,y4, shuffle = False,  train_size = 0.8 )
...
model1 = Model(inputs = [input1, input2], outputs = [output3_1])
...
model1.fit([x1_train,x2_train] , [y1_train],epochs=200, batch_size=1, validation_split=0.25,verbose=1, callbacks=[earlystopping])
...
#4. 평가,예측
loss,mse = model1.evaluate([x1_test,x2_test], y1_test, batch_size=1)
