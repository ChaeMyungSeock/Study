...
'''
parameters = [
    {"n_estimators":np.arange(100, 1000,100), "learning_rate": np.arange(0.2, 0.7, 0.1), 
    "colsample_bytree":np.arange(0.6, 1.0, 0.1), "colsample_bylevel":np.arange(0.6, 1.0, 0.1)}
    ]
# 그리드 파라미터 사용.. 
...
model = GridSearchCV(XGBClassifier(), parameters, cv=kfold, n_jobs=-1)
# model = XGBRegressor()
