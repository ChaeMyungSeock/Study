...
pca = PCA(n_components=12) # 원하는 차원으로 만들어줌
pca.fit(x)
x =pca.transform(x)
...
x_train = x_train.reshape(x_train.shape[0], 2,2,3 )
x_test = x_test.reshape(x_test.shape[0], 2,2,3)
...
# 2. 모델
model = Sequential()
model.add(Conv2D(32,(1,1), input_shape =(2,2,3)))
