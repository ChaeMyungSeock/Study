...
hist = model.fit(x_train, y_train, batch_size=200, epochs=4, validation_split=0.15, callbacks=[earlystopping, checkpoint]) 
...
model = load_model('./model/check05-0.0696.hdf5')


import matplotlib.pyplot as plt

hist = model.fit(x_train, y_train, batch_size=200, epochs=4, validation_split=0.15, callbacks=[checkpoint]) 
