...
# earlystopping 보완  {epoch:02d} => d(정수) epoch 두자리 정수
# {val_loss:.4f} => val_loss를 소수점 4자리까지 float형으로 받음 .hdf5이라는 파일 형태로

checkpoint = ModelCheckpoint(filepath = modelpath, monitor = 'val_loss', save_best_only = True, mode = 'auto')     # save_best_only 좋은것만 저장, mode에 defalut값 존재
hist = model.fit(x_train, y_train, batch_size=200, epochs=10, validation_split=0.15, callbacks=[earlystopping, checkpoint]) 
