...
#.3 모델구성
inceptionv2 = InceptionResNetV2(include_top = False, weights = 'imagenet',input_tensor = Input(shape = (32,32,3)))
# include_top = False로 하면 직접 수정해서  layer 추가할 수 있음

model = Sequential()
model.add(inceptionv2)
model.add(Flatten())
...
hist          = model.fit(x_train, y_train, validation_split=0.3, epochs=5, batch_size=32, verbose=1)#, callbacks=[es,cp,tb])
